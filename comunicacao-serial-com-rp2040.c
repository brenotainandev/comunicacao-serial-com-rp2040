...
#include "comunicacao_serial_com_rp2040.pio.h"
    ...
    while (true) {
        ...
        if (c >= '0' && c <= '9') {  
            ...
            if (posicao > 0) {
                posicao--;
                frase[posicao] = '\0';
                printf("\rFrase atual: %s   ", frase); // Atualiza a frase no Serial Monitor
                fflush(stdout); // Força a atualização no terminal
            ...
            // Exibe no Serial Monitor a frase que está sendo digitada
            printf("\rFrase atual: %s ", frase);
            fflush(stdout); // Força a atualização do terminal
